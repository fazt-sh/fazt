document.addEventListener('DOMContentLoaded', () => {
    ...
    const clPopup = document.getElementById('changelog-popup');
    
    if (versionTag && clPopup) {
        let loaded = false;
        ...
        versionTag.addEventListener('click', (e) => {
            ...
            clPopup.classList.toggle('show');
        });

        // Close on outside click
        document.addEventListener('click', (e) => {
            if (!clPopup.contains(e.target) && e.target !== versionTag) {
                clPopup.classList.remove('show');
            }
        });
        ...
        function loadChangelog() {
            fetch('changelog.json')
                .then(r => r.json())
                .then(data => {
                    const items = data.slice(0, CHANGELOG_LIMIT);
                    let html = '';
                    items.forEach(item => {
                        html += `
                            <div class="changelog-item">
                                <div class="cl-header">
                                    ...
                                    <span class="cl-date">${timeAgo(item.created_at)}</span>
                                </div>
                    ...
                    clPopup.innerHTML = html;
                    loaded = true;
                })
                .catch(() => {
                    clPopup.innerHTML = '<div style="padding:15px; color:#666; font-size:0.8rem;">Failed to load changelog.</div>';
                });
        }
    }
});
