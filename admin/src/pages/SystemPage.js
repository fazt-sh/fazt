...
export default {
  ...
  setup() {
    ...
    const uptimeText = computed(() => {
      const uptime = store.data?.uptime_seconds || 0
      return formatUptime(uptime)
    })

    const memoryUsed = computed(() => {
      return store.data?.memory?.used_mb || 0
    })

    const memoryLimit = computed(() => {
      return store.data?.memory?.limit_mb || 0
    })
    ...
    const vfsCacheMb = computed(() => {
      return store.data?.memory?.vfs_cache_mb || 0
    })

    const dbPath = computed(() => {
      return store.data?.database?.path || 'Unknown'
    })

    const dbConnections = computed(() => {
      return store.data?.database?.open_connections || 0
    })

    const dbInUse = computed(() => {
      return store.data?.database?.in_use || 0
    })

    const queuedEvents = computed(() => {
      return store.data?.runtime?.queued_events || 0
    })

    const goroutines = computed(() => {
      return store.data?.runtime?.goroutines || 0
    })

    const version = computed(() => {
      return store.data?.version || 'Unknown'
    })

    const mode = computed(() => {
      return store.data?.mode || 'Unknown'
    })
