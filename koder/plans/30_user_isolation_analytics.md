...
Redesign the serverless API namespace for automatic user data isolation,
enhanced analytics with user tracking, and improved ID formats for better DX.

## Goals

1. **Automatic user isolation** - `fazt.app.user.*` guarantees privacy
2. **Enhanced analytics** - Track by app_id and user_id
3. **GDPR compliance** - One-call user data deletion
4. **Better ID format** - Stripe-style IDs with fazt prefix
5. **Admin visibility** - Query users, analytics, manage roles

## Non-Goals

- Full RBAC (deferred - see RBAC Notes section)
...
| Collision-proof | 12 chars base62 = 3.2 × 10²¹ combos |
...
// Returns cross-app journey:
[
  { app_id: 'fazt_app_abc...', event: 'login', time: '09:00' },
  { app_id: 'fazt_app_abc...', event: 'pageview', path: '/editor', time: '09:02' },
  { app_id: 'fazt_app_abc...', event: 'purchase', time: '09:15' },
  { app_id: 'fazt_app_xyz...', event: 'login', time: '14:30' },
  { app_id: 'fazt_app_xyz...', event: 'create_note', time: '14:35' }
]
```
...
// Set role (owner can promote to admin, admin cannot promote)
fazt.admin.users.setRole('fazt_usr_Nf4rFeUfNV2H', 'admin')

// GDPR: Delete user and ALL their data
fazt.admin.users.delete('fazt_usr_Nf4rFeUfNV2H')
// Deletes from:
// - fazt.app.user.* storage (all apps on this instance)
// - events table
// - auth_sessions
// - auth_users
```
...
- [ ] Update `generateUUID()` for users to use new format
...
- [ ] Implement `fazt.admin.users.*`
- [ ] Implement `fazt.admin.analytics.*`
...
Full RBAC was discussed but deferred for separate planning.
...
Keep simple roles (owner/admin/user). Apps needing custom permissions
can implement them in their own data:

```javascript
// App-level custom permissions
var perms = fazt.app.user.ds.findOne('permissions', {})
if (perms && perms.canModerate) {
  // Allow moderation
}
```

RBAC can be revisited when multiple apps demonstrate the need.
...
4. All IDs use `fazt_<type>_<12 chars>` format
